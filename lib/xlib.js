...
class Drawable {
    ...
    drawArc(gc, x, y, width, height, angle1, angle2) {
        xlib.XDrawArc(DISPLAY, this._d, gc._gc, x, y,
                        width, height, angle1, angle2);
    }

    drawLine(gc, x1, y1, x2, y2) {
        xlib.XDrawLine(DISPLAY, this._d, gc._gc, x1, y1, x2, y2);
    }

    drawPoint(gc, x, y) {
        xlib.XDrawPoint(DISPLAY, this._d, gc._gc, x, y);
    }

    drawRect(gc, x, y, width, height) {
        xlib.XDrawRectangle(DISPLAY, this._d, gc._gc, x, y, width, height);
    }

    fillArc(gc, x, y, width, height, angle1, angle2) {
        xlib.XFillArc(DISPLAY, this._d, gc._gc, x, y,
                            width, height, angle1, angle2);
    }

    fillRect(gc, x, y, width, height) {
        xlib.XFillRectangle(DISPLAY, this._d, gc._gc, x, y, width, height);
    }
...
//        );
    ...
    resize(width, height) {
        this._checkWindow();
        xlib.XResizeWindow(DISPLAY, this._w, width, height);
    }

    moveResize(x, y, width, height) {
        this._checkWindow();
        xlib.XMoveResizeWindow(DISPLAY, this._w, x, y, width, height);
    }
    ...
    clearArea(x, y, width, height, exposures) {
        this._checkWindow();
        xlib.XClearArea(DISPLAY, this._w, x, y, width, height, !!exposures);
    }
...
function destroyWin(win) {
    // $print("DestroyNotify w =", win._w);
    delete WINLIST[win._w.toString()];
    win.setDrawable(null);
    win._w = null;
    win._callbacks = null;
...
function dispatch(ev) {
    ...
    if (cb) {
        // TODO: if (type == MotionNotify)
        //    ev = compress_motion_events(ev);
...
const _Xfd = $malloc(4);
_Xfd.pack(0, 'i', xlib.ConnectionNumber(DISPLAY));

function event_iteration () {
