...
class Buffer extends Uint8Array {
    ...
    static _pack(b, offset, format, args) {
        if (typeof offset !== 'number')
            throw new TypeError('OFFSET is not a number');
        return $pack(b, offset, format, ...args);
    }

    // unsafe
    pack(offset, format, ...args) {
        if (format === 'J') {
            this.setUint64(offset, args[0]);
            return 8;
        }
        if (format === 'j') {
            this.setInt64(offset, args[0]);
            return 8;
        }
        return Buffer._pack(this._buffer, offset, format, args);
    }
}
