...
Statement.prototype.bind = function (...params) {
    ...
    let offset = bufp.pack(0, 'ps', self._stmt, format);
...
Statement.prototype.next = function () {
    const self = this;
    return $go(sqlite3lib.next, self._stmt)
    .then((data) => {
...
Statement.prototype.each = function (callback, maxRows) {
    ...
    const ptr = $malloc($nullptr.packSize('pI', $nullptr, 0)).gc();
    ptr.pack(0, 'pI', self._stmt, maxRows);
