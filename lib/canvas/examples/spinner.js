...
class Spinner extends Window {
    constructor(parent, x, y, d) {
        ...
        this.spinner.xs = cairo_xlib.surface_create(Window.DISPLAY,
    ...
    draw() {
        ...
        cairo.pop_group_to_source(cr); // OPTIONAL1
        cairo.paint(cr); // OPTIONAL1
    }
    ...
    static _update(c) {
        if (Window.exists(c)) {
            ...
            c.draw();
            XEvent.flush();
            setTimeout(Spinner._update, 100, c);
        }
    }
