...
const CTYPENAME = module.__args[1]; // <- GtkWidget
...
$print(`
const CPROTO = CTYPES['${CTYPENAME}'].proto;
`);
...
funcs.forEach(function(def) {
    ...
    if (!(argc >= 2 && def.params[1].type === TYPEDEFPTR
            && def.params[1].tagname === CTYPENAME)) {
...
`;
    ...
    if (p0.type != VOID)
        ...
        str += '  ';
    if (argc === 2) {
        str += 'lib.' + def.name + '(this);';
    } else {
        str += 'lib.' + def.name + '(this,' + argstr + ');';
    }
...
`;
        ...
        case TYPEDEFPTR:
            ...
            str += `
  if ($r === $nullptr) return null;
  return ctype_to_js('${p0.tagname}', 0, $r);`;
            break;
        default:    /* number */
            str += '\n  return $r;';
    }
    ...
    $print(`CPROTO.${def.name} = function(${argstr}){`);
...
$print('Object.assign(CPROTO, OVERRIDES);');
