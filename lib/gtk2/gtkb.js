...
function gtype_to_js(gtype_name, gtype, ptr) {
    const GObject = GTYPES['GObject'].proto;
    if (ptr === $nullptr)
        return null;
    let gproto = null;
    if (gtype_name === 'GObject') {
        gproto = GObject;
            ...
            && GObject.isPrototypeOf(GTYPES[gtype_name].proto)) {
        gproto = GTYPES[gtype_name].proto;
    } else {
        if (!gtype) {
    ...
    if ((gtype_name in GTYPES)
            && GTYPES['GBoxed'].proto.isPrototypeOf(GTYPES[gtype_name].proto)) {
        // always a copy
        Object.setPrototypeOf(ptr, GTYPES[gtype_name].proto);
