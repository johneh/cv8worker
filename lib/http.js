...
//      value - ArrayBuffer; maybe empty (byteLength == 0) if done is true.
//
//  Usage:
//      let bytesReceived = 0;
//      let perror = (err) => { console.log(err); };
//      resp.readBodyData()
//      .catch(perror)
...
HTTPResponse.prototype.end = function (data) {
    ...
    } else if ($length(data) === null) {
        return Promise.reject(new TypeError('invalid DATA argument'));
    } else {
